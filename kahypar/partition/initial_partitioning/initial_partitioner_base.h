      ...
      if (_context.local_search.algorithm == RefinementAlgorithm::twoway_fm &&
        ...
        switch (_context.partition.objective) {
          case Objective::cut:
            ...
            break;
          case Objective::km1:
            if (_context.local_search.algorithm == RefinementAlgorithm::balance_approaching_kway_fm_km1) {
              refiner = (RefinerFactory::getInstance().createObject(
                         RefinementAlgorithm::balance_approaching_kway_fm_km1,
                         _hg, _context));
              LOG << "balance_approaching_kway_fm_km1.";
              break;
            }
            if (_context.local_search.algorithm == RefinementAlgorithm::imbalance_holding_kway_fm_km1) {
              refiner = (RefinerFactory::getInstance().createObject(
                         RefinementAlgorithm::imbalance_holding_kway_fm_km1,
                         _hg, _context));
              LOG << "imbalance_holding_kway_fm_km1.";
              break;
            }
            refiner = (RefinerFactory::getInstance().createObject(
